...
It uses a local, replicated database composed from *CourseDB* and *StudentDB* to perform fast eligibility checks without making slow, synchronous calls to other services. To avoid races during enrollment, a final
arbiter (*Course Service*) is called upon each enrollment (see [details](diagram-notes.md#)).
