...
* Present a search module with filters and query options.
* Display filtered courses with key information (code, title, semester, teacher, etc.).
* Provide feedback if no courses match the search criteria.
* Allow the user to click on a button to view detailed information via the View a Course feature.

**Course data retrieval responsibilities**
* Query the database to fetch the courses based on User's search filters and query.

**Search and Query Handling Responsibilities**
* Validate user search inputs and selected filters.
* Process keyword and filter queries to generate a filtered list.
* Handle partial matches and synonyms to improve search flexibility.

**Auditing and Analytics Responsibilities**
...
* Cache frequently requested courses or popular search results to reduce database load.
...
1. User navigates to the Course page via a link or the Course Discovery feature.
...
* Checks whether the current user should be able to post a new survey depending on various criteria.If not, a notification is shown.
